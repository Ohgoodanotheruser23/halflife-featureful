...
public:
	...
	void SendMessages(CBaseEntity* pClient);
	bool IsActive() const {
		if (FBitSet(pev->spawnflags, SF_RAIN_ACTIVE))
			return true;
		return false;
	}
	void SetActive(bool active) {
		if (active)
			SetBits(pev->spawnflags, SF_RAIN_ACTIVE);
		else
			ClearBits(pev->spawnflags, SF_RAIN_ACTIVE);
	}
	void SendRain(CBaseEntity* pClient = NULL);
	void SendClearRain();
...
{
	DEFINE_FIELD( CEnvRain, m_intensity, FIELD_SHORT ),
	DEFINE_FIELD( CEnvRain, m_distance, FIELD_SHORT ),
	DEFINE_FIELD( CEnvRain, m_minHeight, FIELD_SHORT ),
	DEFINE_FIELD( CEnvRain, m_maxHeight, FIELD_SHORT ),
...
void CEnvRain::KeyValue(KeyValueData *pkvd)
{
	...
	else if (FStrEq(pkvd->szKeyName, "raindrop_width"))
	{
...
void CEnvRain::SendMessages(CBaseEntity *pClient)
{
	if (!IsActive())
		return;
	SendRain(pClient);
...
void CEnvRain::SendRain(CBaseEntity *pClient)
{
	const int msgType = pClient ? MSG_ONE : MSG_ALL;
	edict_t* pClientEdict = pClient ? pClient->edict() : NULL;

	int flags = pev->spawnflags;
	...
	if (FBitSet(pev->spawnflags, RAIN_BRUSH_ENTITY))
	{
	...
	else
	{
		...
		if (FBitSet(pev->spawnflags, SF_RAIN_LOCALIZED))
		{
...
class CEnvSnow : public CBaseEntity
{
public:
	void Spawn();
	int ObjectCaps( void ) { return CBaseEntity::ObjectCaps() & ~FCAP_ACROSS_TRANSITION; }
	...
	void SendMessages(CBaseEntity* pClient);
	bool IsActive() const {
		if (FBitSet(pev->spawnflags, SF_SNOW_ACTIVE))
			return true;
		return false;
	}
	void SetActive(bool active) {
		if (active)
			SetBits(pev->spawnflags, SF_SNOW_ACTIVE);
		else
			ClearBits(pev->spawnflags, SF_SNOW_ACTIVE);
	}
	void SendSnow(CBaseEntity* pClient = NULL);
	void SendClearSnow();
...
{
	DEFINE_FIELD( CEnvSnow, m_intensity, FIELD_SHORT ),
	DEFINE_FIELD( CEnvSnow, m_distance, FIELD_SHORT ),
	DEFINE_FIELD( CEnvSnow, m_minHeight, FIELD_SHORT ),
	DEFINE_FIELD( CEnvSnow, m_maxHeight, FIELD_SHORT ),
...
void CEnvSnow::KeyValue(KeyValueData *pkvd)
{
	...
	else if (FStrEq(pkvd->szKeyName, "snowflake_min_size"))
	{
...
void CEnvSnow::SendMessages(CBaseEntity *pClient)
{
	if (!IsActive())
		return;
	SendSnow(pClient);
...
void CEnvSnow::SendSnow(CBaseEntity *pClient)
{
	const int msgType = pClient ? MSG_ONE : MSG_ALL;
	edict_t* pClientEdict = pClient ? pClient->edict() : NULL;
	...
	if (FBitSet(pev->spawnflags, SNOW_BRUSH_ENTITY))
	{
		WRITE_COORD(pev->absmin.x);
		WRITE_COORD(pev->absmin.y);
		WRITE_COORD(pev->absmin.z);
	...
	else
	{
		...
		if (FBitSet(pev->spawnflags, SF_SNOW_LOCALIZED))
		{
			WRITE_COORD(pev->origin.x);
			WRITE_COORD(pev->origin.y);
			WRITE_COORD(pev->origin.z);
