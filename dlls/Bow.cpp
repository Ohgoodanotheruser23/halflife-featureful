...
void CBowBolt::BoltTouch(CBaseEntity* pOther)
{
	...
	if (pOther->pev->takedamage)
	{
		...
		pOther->TraceAttack(pev, pevOwner, gSkillData.plrDmgBow, pev->velocity.Normalize(), &tr, DMG_BULLET | DMG_ALWAYSGIB);
		...
		{
		case 0:
			EMIT_SOUND(ENT(pev), CHAN_BODY, "weapons/bow_hitbod1.wav", 1, ATTN_NORM);
			break;
		case 1:
			EMIT_SOUND(ENT(pev), CHAN_BODY, "weapons/bow_hitbod2.wav", 1, ATTN_NORM);
			break;
		}
		Killed(pev, pev, GIB_ALWAYS); //Smokey ;D
...
void CBow::Spawn()
{
	...
	InitDefaultAmmo(BOW_DEFAULT_GIVE);
...
bool CBow::IsEnabledInMod()
{
#ifndef CLIENT_DLL
	return g_modFeatures.IsWeaponEnabled(WEAPON_BOW);
#else
	return true;
#endif
}
...
int CBow::GetItemInfo(ItemInfo* p)
{
	...
	p->pszAmmoEntity = "ammo_bow";
	p->iDropAmmo = AMMO_BOW_GIVE;
	return 1;
}
