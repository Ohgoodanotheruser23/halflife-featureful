...
void CGay::AlertSound(void)
{
	...
	EmitSoundDyn(CHAN_VOICE, pAlertSounds[RANDOM_LONG(0, ARRAYSIZE(pAlertSounds) - 1)], 1.0, ATTN_NORM, 0, pitch);
}
...
void CGay::IdleSound(void)
{
	...
	EmitSoundDyn(CHAN_VOICE, pIdleSounds[RANDOM_LONG(0, ARRAYSIZE(pIdleSounds) - 1)], 1.0, ATTN_NORM, 0, 100 + RANDOM_LONG(-5, 5));
}

void CGay::AttackSound(void)
{
	// Play a random attack sound
	EmitSoundDyn(CHAN_VOICE, pAttackSounds[RANDOM_LONG(0, ARRAYSIZE(pAttackSounds) - 1)], 1.0, ATTN_NORM, 0, 100 + RANDOM_LONG(-5, 5));
}
...
void CGay::HandleAnimEvent(MonsterEvent_t* pEvent)
{
	...
	case GLENN_AE_ATTACK_RIGHT:
	{
		...
		if (pHurt)
		{
			...
			EmitSoundDyn(CHAN_WEAPON, pAttackHitSounds[RANDOM_LONG(0, ARRAYSIZE(pAttackHitSounds) - 1)], 1.0, ATTN_NORM, 0, 100 + RANDOM_LONG(-5, 5));
		}
