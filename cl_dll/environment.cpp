...
static void AddRandomHorizontalDistance(Vector& vecOrigin, float distance, bool distanceIsRadius)
{
	if (distanceIsRadius)
	{
...
Vector WeatherData::GetRandomOrigin(const Vector& weatherOrigin) const
{
	...
	else
	{
		...
		AddRandomHorizontalDistance(vecOrigin, distance, DistanceIsRadius());
...
void CEnvironment::Update()
{
	...
	for (auto& rain : m_rains)
	{
		...
		{
			UpdateRain(rain);
			rain.updateTime = clientTime + 0.3f;
		}
	}
	for (auto& snow : m_snows)
	{
		...
		{
			UpdateSnow(snow);
			snow.updateTime = clientTime + 0.7f;
		}
	}
