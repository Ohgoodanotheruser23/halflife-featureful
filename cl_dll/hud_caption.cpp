...
void CHudCaption::CalculateLineOffsets(Subtitle_t &sub)
{
	...
	if (CHud::ShouldUseConsoleFont())
	{
		...
		{
			const int width = CHud::GetMessageConsoleWidth(str + boundaries[startWordIndex].wordStart, boundaries[j].wordEnd - boundaries[startWordIndex].wordStart);
			if (width > xmax) {
				if (j == startWordIndex) {
...
int CHudCaption::Draw(float flTime)
{
	...
	for (i=0; i<sub_count; ++i)
	{
		...
		for (j=0; j<subtitles[i].lineCount; ++j)
		{
			...
			if (CHud::ShouldUseConsoleFont())
			{
				lineWidth += CHud::GetMessageConsoleWidth(subtitles[i].caption->message + subtitles[i].lineOffsets[j], subtitles[i].lineEndOffsets[j] - subtitles[i].lineOffsets[j]);
			}
			else
			{
	...
	for (i=0; i<sub_count; ++i)
	{
		...
		for (j=0; j<sub.lineCount; ++j)
		{
			...
			DrawUtfString( xpos, ypos, xmax, sub.caption->message + sub.lineOffsets[j], sub.r, sub.g, sub.b, sub.lineEndOffsets[j] - sub.lineOffsets[j] );
			ypos += lineHeight;
		}
